...
import citreaService from '../services/citrea_service';
...
const LeaderboardScreen = ({ onShowStartScreen }) => {
  ...
  const checkWalletConnection = async () => {
    const result = await citreaService.checkConnection();
    if (result.success) {
  ...
  const loadLeaderboard = async () => {
    ...
    try {
      const result = await citreaService.getTopScores(20);
      if (result.success) {
        setLeaderboard(result.data);
        
        // Find player's rank if wallet is connected
        if (walletAddress) {
          const rank = result.data.findIndex(entry => 
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
          ...
          <p style={{
            ...
            Scores stored permanently on Citrea blockchain
          </p>
          {playerRank && (
            <div style={{
              marginTop: '15px',
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          background: isCurrentPlayer ? 'rgba(0, 255, 136, 0.05)' : 'transparent',
